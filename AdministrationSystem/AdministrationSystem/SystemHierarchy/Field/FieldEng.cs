...
namespace AdministrationSystem.AdministrationSystem.SystemHierarchy.FieldEng
{
    [TestFixture]
    [Parallelizable(ParallelScope.Fixtures)]
    [AllureNUnit]
    [AllureSuite("Field Page")]
    [AllureFeature("System Hieracrchy Page")]
    [AllureEpic("Administration System features")]
    public class TestsFieldEngPage
    {
        ...
        public void FieldPage()
        {
            ...
            // Field >>
            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.XPath("//*[@id=\"app\"]/div[2]/aside/div[2]/nav/ul/li[1]/ul[2]/li/ul[4]/li/a")));
            Thread.Sleep(5000);
            driver.FindElement(By.XPath("//*[@id=\"app\"]/div[2]/aside/div[2]/nav/ul/li[1]/ul[2]/li/ul[4]/li/a")).Click();
            ...
            Thread.Sleep(5000);
            IWebElement PAGE = driver.FindElement(By.XPath("//body[1]/div[3]/div[1]/div[2]/form[1]/div[1]/div[1]/div[3]/span[1]"));
            PAGE.Click();

            String[] PAGEText = { "User", "Role", "Archiving Tree", "Form", "Page Security", "User Role", "Setting Tender", "OfferingTenderItem", "InvoicePayment", "Fine Of Installation", "HCourse Payment Voucher", "MainGrant" };

            if (PAGEText.Length > 0)
            {
                Random rand = new Random();
                int randNumber = rand.Next(0, PAGEText.Length);
                Console.WriteLine(PAGEText[randNumber]);
                PAGE.SendKeys(PAGEText[randNumber] + Keys.Enter);
            }

            driver.FindElement(By.Id("Arabic-name")).SendKeys("النقطة المرجعية");
            driver.FindElement(By.Id("English-name")).SendKeys("ParentId");
            driver.FindElement(By.Id("NAME")).SendKeys("orderIcon");
            ...
            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.Id("Arabic-name")));
            Thread.Sleep(3000);
            driver.FindElement(By.Id("Arabic-name")).Clear();
            driver.FindElement(By.Id("Arabic-name")).SendKeys("الشركة");
            Thread.Sleep(5000);
            ...
            driver.FindElement(By.Id("English-name")).SendKeys("Id");
            Thread.Sleep(3000);
            driver.FindElement(By.XPath("//button[normalize-space()='EDIT']")).Click();
            Thread.Sleep(5000);
            ...
            Assert.Multiple(() =>
            {
                Assert.That(cardTitleText, Is.EqualTo("Field"));
            });
